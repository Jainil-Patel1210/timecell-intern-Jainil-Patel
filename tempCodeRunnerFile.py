...
try:
    ...
    print("⚠  google-generativeai not installed. Briefing feature disabled.")
...
def render_dashboard(
    ...
    if briefing:
        print(f"\n  {'─' * (w - 4)}")
        print("  AI DAILY BRIEFING (Gemini)")
        print(f"  {'─' * (w - 4)}")
...
def main() -> None:
    # ---- Configuration ----
    REFRESH_SECONDS = 30          # refresh interval (lower = more API calls)
    ALERT_THRESHOLD = 5.0         # % price change to trigger an alert
    BRIEFING_EVERY  = 3           # generate a new AI briefing every N cycles
    ...
    while True:
        ...
        # Generate AI briefing on first cycle and then every BRIEFING_EVERY cycles
        if GEMINI_AVAILABLE and GEMINI_API_KEY and (cycle == 1 or cycle % BRIEFING_EVERY == 0):
        ...
        # Save current prices as previous for next cycle's alert comparison
